#!/usr/bin/env node
/**
 * Builds pages/testimonials.html with all 73 canonical reviews,
 * filter pills, and complete JSON-LD schema.
 */
const fs = require('fs');
...
function getApplianceLabel(r) {
  if (!r.appliance) return 'Appliance Repair';
  const map = { washer:'Washer Repair', dryer:'Dryer Repair', refrigerator:'Refrigerator Repair',
...
const reviews = pool.reviews.filter(r => r.bodyStatus === 'complete' && r.nameFlag !== 'non-person');
...
const reviewSchemaEntries = reviews.map(r => `      {
        "@type": "Review",
...
// Read current page nav/footer sections we want to preserve
const currentPage = fs.readFileSync(path.join(__dirname, '../pages/testimonials.html'), 'utf8');

// Extract nav section (between <body> and <!-- PAGE HERO / page-hero section start)
...
<script>
  ...
  <meta name="description" content="73 verified 5-star Google reviews for Universal Appliances Repair in Orange County, CA. See what customers say about our refrigerator, washer, dryer, and dishwasher repair." />
  ...
  <meta property="og:description" content="73 verified 5-star Google reviews for Universal Appliances Repair in Orange County, CA." />
  ...
  <meta name="twitter:description" content="73 verified 5-star Google reviews for Universal Appliances Repair in Orange County, CA." />
...
${navHtml}  <div class="page-hero">
  ...
  <section class="section-white" style="padding:32px 24px;border-bottom:1px solid #f0f0f0;">
    <div class="container-lg" style="display:flex;justify-content:center;gap:60px;flex-wrap:wrap;text-align:center;">
      <div>
        <div style="font-size:32px;font-weight:800;color:#e84c1e;letter-spacing:-1px;">&#9733; 5.0</div>
...
console.log(`Written pages/testimonials.html with ${reviews.length} review cards.`);
