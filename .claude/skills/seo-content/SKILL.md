...
- **Service-fee references — tiered rule (2026-05-14):** Two distinct contexts, never mixed:
  ...
  2. **"Our company fee" statement** (policy framing, anywhere in body copy, FAQ answers, schema, step descriptions) → **geography-tiered by county, flat within each tier** (owner, 2026-07-07; LA County confirmed uniform 2026-07-30):
     ...
     | **Riverside County** | **`$99`** | **all Riverside County cities**, western (Corona, Norco, Eastvale) and eastern / Coachella Valley (Riverside, Palm Springs, Cathedral City, Rancho Mirage, Palm Desert) alike |

     **There is now ONE company fee: `$99`, everywhere we serve, for every city and every brand.**

     **The fee is flat everywhere and never varies by city, county or brand.** LA County is a **single `$99` tier: one fee for every city and every brand in the county** (owner decision 2026-07-30). ❌ Never write a per-brand or per-LA-city fee split. A Sub-Zero in Beverly Hills, a Wolf range in Santa Monica, and a Whirlpool washer in Long Beach are all **`$99`**.

     Note this is separate from **service scope**: the four premium cities are marketed for premium brands only (`pages/luxury-appliance-repair-los-angeles-ca.html`). **Scheduling does NOT differ** — a technician is based in LA, so same-day is available there when that schedule is open, with next-available (usually within a day or two) as the honest fallback. Brand scope is the only thing that makes those four cities different: not the fee, not the scheduling. **Scheduling is set by COVERAGE, never by drive length.** Technicians are based in Los Angeles County and in Riverside County as well as Orange County. Orange County and Los Angeles County are same-day whenever the assigned technician's schedule is still open, with next-available (usually within a day or two) as the honest fallback; never write that either county cannot do same-day. **Riverside County is split as of 2026-08-22, and the two halves are written differently:** the Riverside technician is based on the Riverside side of the county and works EAST toward the Coachella Valley, so Riverside city, Palm Springs, Cathedral City, Rancho Mirage and Palm Desert are same-day when that schedule is open. Whether he covers WESTERN Riverside (Corona, Norco, Eastvale) is an open owner question, so those cities default to next-available, usually within a day or two, with same-day only when a technician is already routed nearby. Do not promise western-Riverside same-day until that question is answered, and do not "restore" the pre-2026-08-22 wording that said no county is next-available-only. The limit is coverage, not distance: never justify a scheduling limit by the length of the drive, and never reintroduce a distance-based fee (see the History note below).

     **History:** Riverside County billed `$120` from 2026-07-10 until **2026-08-22**, when the owner collapsed every tier into a uniform `$99`. The retired rule said the `$120` tier was "intentional (do NOT reconcile it down to `$99`)" because it funded a dedicated Riverside technician and filtered low-budget jobs at distance. **That reasoning is retired and the instruction is now inverted: there is no `$120` tier and no distance premium anywhere.** A `$120` in a *company-fee* statement is now a defect. It remains valid inside a **repair cost estimate** ("$120 to $300 for a drain pump"); ~29 files legitimately contain it that way, and the CI check keys on the word "fee" precisely so those are untouched.
     ...
     **History:** until 2026-08-15 this rule said Riverside County was next-available-only "because it has no local technician". The owner corrected that on 2026-08-15: there are local technicians in both LA County and Riverside County, and a job is either dispatched to one of them or covered by our own technician. The old claim shipped on the homepage FAQ, the Corona hub (body copy, hero and CTA headlines, and its `description`/`og:description`/`twitter:description` meta tags), the Service Areas page and llms.txt, and was removed from all of them in the same pass. Note the meta tags specifically: they are what a search snippet renders, they are not checked for content accuracy by anything in `npm test`, and they were the last surface found. Do not reintroduce it. **Superseded again 2026-08-22, and the distinction matters:** WESTERN Riverside (Corona, Norco, Eastvale) is next-available by default once more, but for a different reason than the 2026-08-15 version gave. It is not that no Riverside technician exists; one does, and he covers the eastern half same-day. It is that his coverage of the western cities is an unresolved owner question. Do not collapse the two explanations together, and do not cite the old "no local technician" reasoning, which remains wrong.

  Never state our fee as a *range* (it is a single flat value everywhere). Never put a company-fee value into a cost-table row (that row stays the brand-tiered *range* above). These contexts must stay separate.
...
- **Aggregate rating in schema.** Once 6+ real reviews exist, add `AggregateRating` JSON-LD pulled from GBP. `AggregateRating` is required on the homepage and every hub page, and its `reviewCount` mirrors what the site currently claims: `data/testimonials.json` `_meta.sources.google.publishedCount` (read the live value from that field). `publishedCount` is promoted from the live GBP listing total (`totalReviewsOnListing`) only during a weekly publish batch (`npm run build:review-counts -- --publish`); the separate `capturedCount` field tracks how many reviews are transcribed into the pool, and all three legitimately differ. The `content-integrity` "review-count" check validates every page against `publishedCount`, so never hardcode a different number. Yelp is **not** folded into `AggregateRating` (4-review pool, 4.0 average, mixing sources is discouraged by Google's structured-data guidance). If Yelp is surfaced as a trust signal, do it as a separate visual element ("See our Yelp reviews [link]"), not in schema.
