...
- **No phone numbers in post body text.** For new job-placement plans, use **Learn more** with a link to the relevant published page on `fixappliancesfast.com` (owner preference, 2026-09-07). This replaces the earlier Call now-only house preference; it does not change Google's phone-number restriction. Use Call now only if the owner requests it for a particular post. Google supports website action links and the `LEARN_MORE` action: `support.google.com/business/answer/7342169` and `developers.google.com/my-business/content/posts-data` (read 2026-09-07).
...
- ⚠️ **Yelp's Content Guidelines ban AI-drafted content outright, and the ban explicitly covers revision as well as authorship.** Re-verified against `yelp.com/guidelines` on **2026-08-19**, which now reads: *"you shouldn't use third-party AI tools and chatbots to create reviews or other content, including using such tools to draft or revise content."* The earlier note here recorded the scope of "other content" as unconfirmed; the live page has since resolved it, and the `draft or revise` clause is the operative part. **A human writes and owns any Yelp text. We supply facts, never finished copy, and never a "cleaned-up" pass over the owner's own draft either** — revising is named as prohibited, so offering to tighten a sentence the owner wrote is not a loophole. This binds every Yelp surface that carries text, including the photo description field below.
...
**Partial re-verification 2026-08-19** (two pages only, everything else in this file still dates from 2026-07-30): `support.google.com/business/answer/7213077` still carries the phone-number ban verbatim and still limits its categorical promotions ban to hotels, incorporating Google's broader prohibited-content policies by reference — the GBP section above holds unchanged. `yelp.com/guidelines` has **tightened**: the AI clause now names revision explicitly, and the review-solicitation ban appears on that page in its own words. Both changes are folded into the Yelp bullets above. **Instagram was NOT re-checked on this date** — its help pages are JS-rendered and need a real browser, so the 5-hashtag cap and the caption limits still rest on the 2026-07-30 browser verification.

GBP: `support.google.com/business/answer/7213077`, `/7342169`, `/7400114`, `/6103862`. Yelp: `yelp.com/guidelines`, `biz.yelp.com/support-center/Reviews/Best_Practices/Don-t-Ask-for-Reviews/en-US`, `.../What-is-Yelp-Connect`, `.../Yelp-Deals-Gift-Certificates-Check-In-Offers-FAQ-for-Business-Owners`. Instagram: `help.instagram.com/351460621611097`, Meta Community Standards + Advertising Standards at `transparency.meta.com`.
