...
If Playwright MCP is not loaded, the review is automatically **BLOCKED** — state this explicitly and do not approve. Do not substitute impeccable for visual-review; they check different things.

Any FAIL item from `/visual-review` is a **blocker** — same weight as a broken link.
...
**Visual review blockers (FAIL = do not merge):**
- [ ] `/visual-review` was run and output is present in the PR description or review — missing output is a FAIL
- [ ] No FAIL items at desktop viewport (1440×900)
- [ ] No FAIL items at mobile viewport (375×812)
- [ ] Hamburger nav toggles correctly on mobile (`aria-expanded` flips, drawer opens and closes)
- [ ] Sticky Call/Book bar visible on mobile
- [ ] No horizontal scroll at either viewport
- [ ] Hero h1 ≤ 34px on mobile, ≥ 40px on desktop
