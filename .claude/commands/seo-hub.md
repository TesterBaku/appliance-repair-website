...
- **Target**:
  ...
  - For `city`: which city? (must be from the Primary or Secondary list in `.claude/skills/seo-content/SKILL.md`)
...
1. **Read the rules first.** Open and read:
   - `.claude/skills/seo-content/SKILL.md` — brand canonicalization, AI answer block template, FAQ count requirements, schema templates
   - `.claude/skills/mobile-design/SKILL.md` — mobile breakpoints, hamburger, sticky CTA bar
...
5. **Brand intel** (service hub only) — list which brands the top competitors prominently service. Cross-reference with our supported list in `.claude/skills/seo-content/SKILL.md` (Whirlpool, GE, Samsung, LG, Sub-Zero, Wolf, Bosch, Viking, KitchenAid, Maytag, Frigidaire, Kenmore, Thermador, Miele, Dacor). Pick **6–10 brands to feature on the page**, with a short note on common issues for the top 3 (e.g., "Sub-Zero compressor sealed-system repairs", "Samsung ice-maker recall").
...
Section outline:
  ...
  7. FAQ ([N] questions)
  8. Testimonials (3 from `data/testimonials.json`, pool-filtered)
  9. CTA box
...
Use the schema templates in `.claude/skills/seo-content/SKILL.md` as the source of truth — copy them, fill in the variables, do not invent fields.
...
2. **AI answer block** — plain prose, 60–110 words, names entity / services / area / phone / booking. Use the template in `.claude/skills/seo-content/SKILL.md`, adapted:
...
6. **What this typically costs** — *only if cost content is approved for this run* (default: skip per `.claude/skills/seo-content/SKILL.md`)
7. **FAQ section** — minimum count per type:
   - Service hub: **8+** FAQs
   - City hub: **5+** FAQs
   - Each Q&A becomes a `mainEntity` in FAQPage schema
8. **Testimonials** — **3 real reviews** pulled from `data/testimonials.json`. Follow the full selection algorithm in `.claude/skills/testimonial-selection/SKILL.md`:
...
Apply `.claude/skills/mobile-design/SKILL.md` in full:
...
Apply the Bug Fix Workflow from `.claude/skills/git-workflow/SKILL.md`. Do not proceed until both commands exit 0.
...
- Never write "Fix Appliances Fast" as a brand name (it's a URL only — see `.claude/skills/seo-content/SKILL.md`)
