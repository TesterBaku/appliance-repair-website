...
**Active routine:** `trig_015WszAyFWDSSsyfP9GF7F2u` — runs Mon/Wed/Fri at 13:00 UTC (6 AM PDT)
Manage at: https://claude.ai/code/routines/trig_015WszAyFWDSSsyfP9GF7F2u
...
1. Read all existing `article-*.html` filenames and `.claude/skills/seo-content/SKILL.md` to identify the top 5 uncovered city × appliance combinations that look most promising.
...
### SEO elements (required — see `.claude/skills/seo-content/SKILL.md`)
...
Run `npm run build:search` after writing the article. It regenerates the committed Pagefind blog search index (`pagefind/`) so the new article is findable in the blog search box. Commit the `pagefind/` changes in the same commit (same discipline as the sitemap). See the "Search index" standing rule in `AGENTS.md`.

### Blog counts
Add the new card to `pages/blog.html` (and the matching `pages/blog/<category>.html` lander) with the correct `data-category`, then run `npm run build:blog-counts` — do NOT hand-edit the search placeholder, the category pills, or the `N <Cat> Articles` headers; the script derives them all from the live card counts. `npm test` runs `sync-blog-counts --check` and fails on drift.

### Commit
```
npm run build:sitemap
npm run build:search
npm run build:blog-counts
git add articles/[slug].html pages/blog.html pages/blog/ sitemap.xml pagefind/
...
Apply the Bug Fix Workflow from `.claude/skills/git-workflow/SKILL.md`:
...
- **To:** the owner's email — run `echo "$OWNER_EMAIL"` to read it (set in the routine environment; see `.env.example`). Do not hardcode a literal address in this committed file.
