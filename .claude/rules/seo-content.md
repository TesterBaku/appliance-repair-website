...
- **Aggregate rating in schema.** Once 6+ real reviews exist, add `AggregateRating` JSON-LD pulled from GBP. `AggregateRating` is required on the homepage and every hub page, and its `reviewCount` mirrors the public GBP listing total in `data/testimonials.json` `_meta.sources.google.totalReviewsOnListing` (read the live value from that field; the separate `capturedCount` field tracks how many of those are transcribed into the pool, and the two legitimately differ). The `content-integrity` "review-count" check validates every page against `totalReviewsOnListing`, so never hardcode a different number. Yelp is **not** folded into `AggregateRating` (4-review pool, 4.0 average, mixing sources is discouraged by Google's structured-data guidance). If Yelp is surfaced as a trust signal, do it as a separate visual element ("See our Yelp reviews [link]"), not in schema.
