...
- **Location label: "Orange County, CA" by default.** Google Business Profile reviews don't expose the reviewer's city. Use "Orange County, CA" as the location label on every testimonial unless (a) the reviewer's profile or review text names a specific OC city, or (b) the job-photo corroborated attestation exception applies: the business holds its own photo of that job in `images/real/business/` AND the technician or owner confirms the city, with both recorded in the record's `_note`. Both parts are required; either alone falls back to the default. Never invent a city for a reviewer, and never infer one from a name or profile photo. Full wording in `testimonial-selection.md`. Source-of-truth for review data is `data/testimonials.json`, which now spans Google and Yelp; the location-label rule is source-specific (see `testimonial-selection.md`).
...
- **Aggregate rating in schema.** Once 6+ real reviews exist, add `AggregateRating` JSON-LD pulled from GBP. `AggregateRating` is required on the homepage and every hub page, and its `reviewCount` mirrors the public GBP listing total in `data/testimonials.json` `_meta.sources.google.totalReviewsOnListing` (currently **97**; 96 of those are transcribed into the pool, `capturedCount`). The `content-integrity` "review-count" check validates every page against `totalReviewsOnListing`, so never hardcode a different number. Yelp is **not** folded into `AggregateRating` (4-review pool, 4.0 average, mixing sources is discouraged by Google's structured-data guidance). If Yelp is surfaced as a trust signal, do it as a separate visual element ("See our Yelp reviews [link]"), not in schema.
