...
- ✅ **Yelp's photo upload HAS a description field** (owner-confirmed from the Yelp business UI, 2026-08-11). Unlike GBP, where the photo is the whole post, a Yelp photo can carry text. Two consequences: (a) never assume a platform lacks captions because GBP does; (b) that description **is Yelp text**, so the AI-authorship line below governs it — a human writes it. Supply the job facts (appliance, fault, outcome, city, setting) and let the owner write the sentence. Same content limits as everything else on Yelp: city level only, nothing promotional, and claim only what the photo actually shows.
