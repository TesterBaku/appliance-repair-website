...
npm test                   # Link check + HTML integrity + content integrity + CSS vars + repo hygiene (large-file guard) + formspree-guard (contact-form route-stub check) + partial drift check (footer/nav) + site.js drift check + blog-count drift check + article-byline drift check + review-count drift check
...
npm run build:search       # Rebuild the Pagefind blog search index from scratch (run after adding/removing/renaming an article)
...
npm run photos:import -- <manifest.json> # Preview a job-photo batch; add --write to import (docs/job-photo-import.md)
npm run reviews:check      # Read-only review-pool validation; add --batch <manifest.json> for screenshot provenance and quote eligibility
npm run test:helpers       # Focused photo-import and review-batch helper tests (also run by npm test)
```
...
`pages/blog.html` search is powered by **Pagefind** full-text search over article bodies (`pagefind/`, a committed static index). The inline blog-search IIFE calls the Pagefind JS API (`import('../pagefind/pagefind.js')`), maps result URLs back to the existing `.blog-card`s, and renders them in relevance order; it falls back to the legacy substring match over card title/excerpt/badge if the bundle can't load. The index is generated by `scripts/build-search-index.js` (`npm run build:search`), which runs `pagefind --site . --glob "articles/**/*.html"` and excludes shared chrome (nav/footer/sticky bar/related cards) via `--exclude-selectors` so relevance reflects article content, not the repeated nav city list. **The build is a full rebuild, not an update.** Pagefind content-hashes every chunk it emits and never deletes a superseded one, so a script that ran it in place made `pagefind/` grow with every run: by 2026-08-21 it held 222 `.pf_fragment` files for 75 live articles, 313 `.pf_index` files against 13, thirty `pagefind.en_*.pf_meta` files where there should be one, and 12 MB in place of 1.6 MB. None of it was ever served (`pagefind-entry.json` and the live `.pf_meta` reference only current chunks), which is why it went unnoticed for so long: it is invisible to the reader and to every gate. Fixed in PR #769 (P6-38) by building into a temp directory and swapping it in only after pagefind exits 0, so a failed build cannot destroy the committed index. Do not "simplify" that into deleting `pagefind/` before the run. The 7 `pages/blog/*.html` category pages are static landers (no search box — they just list their category's cards under an `N <Cat> Articles` header). `pagefind/` is committed output (like `sitemap.xml`); `.nojekyll` at the repo root keeps GitHub Pages from touching it.
...
- `test/` — the six `npm test` checks (`links.js`, `html-integrity.js`, `content-integrity.js`, `css-vars.js`, `repo-hygiene.js`, `formspree-guard.js`) plus `faq-parity-baseline.json` and `tap-target-baseline.json`, the Playwright screenshot runner (`screenshot.js`), the Playwright functional spec (`functional.spec.js`), and the static server (`serve.js`) the functional suite auto-starts on :8788. `formspree-guard.js` scans every `test/**/*.spec.js` `test()` block and fails if its body (never its title string) references a submit-candidate token for the contact form (a quote-anchored `#form-submit` or `#contact-form` selector literal, backtick, single, or double quotes; the word `formspree` inside any quoted string; or `Send Message` inside any quoted string), which together also catch `page.locator('#form-submit').click()`, a variable-assigned locator clicked later, `page.press(...)`, `.evaluate()`/`.dispatchEvent('submit')`, an alternate CSS selector built off the form's `action` attribute, and `page.getByRole('button', { name: 'Send Message' })`, without that reference being preceded, in the same block, by a `page.route('**formspree.io/**', ...)` stub: the enforced version of the discipline `functional.spec.js` already follows by hand, so a future test (or a subagent) can no longer submit the live contact form for real (P6-47). It is a text scan, not a JS parser, so a selector assembled at runtime (string concatenation, template interpolation of a variable, or a helper function returning the selector) never appears as one contiguous quoted literal and is a stated, undetected gap. `functional.spec.js` derives `CITY_HUBS` from the filesystem (`fs.readdirSync` over `pages/`, anchored `^appliance-repair-.*-ca\.html$`) rather than a hand-maintained array, so a new city hub is covered the moment its file exists — **do not reintroduce a literal list.** The anchor is load-bearing: `luxury-appliance-repair-beverly-hills-ca.html` contains `appliance-repair-beverly-hills-ca.html` as a substring, so an unanchored pattern sweeps the 5 luxury pages into a suite whose assertions they cannot pass. `PREMIUM_HUBS` and `BRAND_HUBS` stay explicit lists on purpose — those pages take different assertions. The hand-maintained version had drifted to 21 entries against 32 files, and that gap already shipped a bug: `appliance-repair-long-beach-ca` went out with 3 dead `tel:` links partly because it sat outside the list, so the suite's own phone-consistency test never ran against it. `repo-hygiene.js` enumerates git-tracked files only (`git ls-files`) and fails on any tracked file over 5 MB (with a narrow, path-specific allowlist for legitimate large assets) or carrying a banned extension (`.pdf`, `.zip`, and other archive/installer formats); it was added after a 13.6 MB research PDF was swept into a commit by `git add -A` on 2026-08-02. `npm test` additionally runs the five `scripts/build/*.js --check` drift guards listed above (`inject-partials`, `inject-site-js`, `sync-blog-counts`, `sync-article-bylines`, `sync-review-counts`), plus `check-agents.js --check` (documented separately below).
...
**Search index** — on any PR that adds, removes, or renames an `articles/article-*.html` file, also regenerate the Pagefind blog search index with `npm run build:search` and commit the resulting `pagefind/` changes (same discipline as the sitemap). The committed index can otherwise go stale and the new article won't be findable in blog search. Never hand-edit anything under `pagefind/`. Because the build now replaces the directory wholesale, expect the diff to contain deletions as well as additions, and expect a rename or a content edit to churn chunks for articles you did not touch: the hashes are content-derived. Stage it with an explicit `git add pagefind`. **Verify a rebuild functionally, never by file count.** A `.pf_fragment` is compressed binary and greps as noise, which produced a false "not indexed" reading during PR #699; query 3-4 terms in the browser against `pages/blog.html` and confirm the results resolve to the right cards.
...
**CSS background-image heroes** (hub pages) require an explicit `<link rel="preload" as="image">` in `<head>` — the browser preload scanner cannot discover them from CSS. Use `scripts/add-hero-preload.mjs` to backfill. **Enforced since 2026-08-20 by the `hero-preload` check in `test/content-integrity.js`** (check 23): every image URL declared on any `.hub-hero-bg` rule block — the `background-image:` longhand or the `background:` shorthand, every block on the page, so a base rule plus a `@media` mobile override are both covered — must carry a preload `<link>` whose `href` resolves to that same file. It resolves both the CSS `url()` and the `href` against the page's own directory rather than comparing raw strings, so two different relative paths to one file match and two different files sharing a basename do not. Until then the rule was documented but unchecked, and `pages/services.html` had quietly regressed: its hero loaded `hero-homepage.webp` while its only preload was for `shared.css`. Live coverage: 69 pages.

**Mobile tap targets (44px)** — every discrete clickable control must be at least 44px tall on a phone. **Enforced since 2026-08-20 by the site-wide tap-target sweep in `test/functional.spec.js`** (`npm run test:functional`, P6-61): it loads every `.html` page in the tree at 375×812, waits for `document.fonts.ready`, and measures `getBoundingClientRect().height` on every visible non-`inline` `<a>`. Ratcheted against `test/tap-target-baseline.json`, keyed by class rather than by page so a new city hub reusing an already-declared debt class is not friction; the baseline may only shrink. Live coverage: ~4,900 anchors across 173 pages, **0** baselined debt classes: P6-62 paid ten down on 2026-08-21 and P6-50 (PR #811, 2026-09-04) the last two (`.hero-rating`, `.brand-pill`), so the sweep now runs with an empty baseline. The sweep is ~11s.

> ⚠️ **It is a browser measurement, and it must stay one.** This gate was first written as a static `content-integrity` check that parsed each page's CSS and computed `padding + border + line-box`. It was wrong in both directions at once. It reported **61 broken pages where the browser found 5**, because `.hub-cta-row` is a flex container whose default `align-items: stretch` was silently pulling the short `.btn-outline-white` up to the height of its sibling `.btn-white`, which does carry `min-height: 44px`. At the same time it was blind to `.hub-link`, `.blog-link`, `.job-card-link`, the Leaflet map controls and 574 unclassed anchors — most of the real debt — because it only understood bare single-class selectors. Rendered height depends on the whole cascade plus flex/grid layout, and no text scan resolves that. **Do not "simplify" this back into `npm test`.** The same conclusion is already recorded twice in this repo: the mobile-occlusion suite's own comment ("The behaviour is what matters, so measure the behaviour, everywhere") and the `contrast-aa` docblock, which defers its unresolvable `var()`/`rgba()` cases to in-browser probes in exactly this spec.
>
> **Measure after fonts load.** Inter loads late and fallback metrics are shorter, so a control near the boundary reads short before the swap and fine after. `404.html`'s button measures 43px pre-swap and 45px post-swap; without the `document.fonts.ready` wait the P6-61 proof run reported 11 findings on the first pass and 10 on the retry, from that one button.
>
> **It measures the HIT AREA, not the box.** Three card patterns here put the tap target on the card rather than the label, via `::after { position: absolute; inset: 0 }` over a positioned card: `.blog-link`, `.job-card-link`, `.read-more`. Their own boxes are 15–35px while the real target is 327×498, 311×378 and 327×704. The sweep walks to the positioned ancestor — and still **fails when that ancestor is missing or too small**, rather than exempting the class outright. That distinction is load-bearing: it is what caught the 7 `pages/blog/*.html` category pages, which had copied the card markup without `position: relative` or the overlay, leaving identical-looking cards card-clickable on the blog index and a 15px text link everywhere else (P6-62).
>
> **`display: inline` does NOT blanket-exempt.** It exempts a link in a run of text (WCAG 2.5.8), except for known discrete controls (`.footer-links`, `.nav-links`, `.nav-drawer`, `.sticky-mobile-bar`). The blanket version shipped a false negative: `.footer-links a` on all 75 articles rendered 15px AND `display: inline`, and the gate called them clean. Breadcrumbs are exempted by name, because `.breadcrumb` is a flex container and its links therefore never compute as inline.
>
> **Known gaps, stated rather than implied:** `<a>` only — `<button>` is not covered, so `pages/testimonials.html`'s filter pills (P6-53) sit outside this gate; `display: inline` links are exempt per WCAG 2.5.8 and P6-50; and it checks height only, not WCAG 2.5.8's 24×24 both-axes rule.
...
- `tasks/backlog.md` — single source of truth for all open work: exhaustive, so nothing is lost, but
  not prioritized and not re-verified. For *what to do next*, read `tasks/priorities.md`
...
- `tasks/priorities.md`: the owner-approved three-tier view (high / medium / low-or-owner-blocked)
  of the backlog, consolidated 2026-09-02 against master 7d2ec1e. Supersedes `tasks/top_3_tasks_priority.md`
  (archived) for "what to do next". Read it before proposing work, and **reconcile any item you
  intend to start against `git log` first**: four consecutive rounds have found the source documents
  wrong, including one item marked queued whose shipping commit was already the HEAD of master.
...
> **Why the tier was removed.** PR #759 claimed it for a single `<link rel="preload">` added to `<head>`, on the reasoning that the tag has no rendered surface. The reviewer blocked it: the rule's bright line was "no new elements", and a `<link>` is one. What makes this worth recording is not that the call was wrong but *how* — the tool was named honestly, and the **scope** was argued down by swapping the written test ("is it a new element?") for a plausible-sounding one ("does it render?"). That reaches P6-31's destination, a required gate not running, by a route that reads as reasonable. A boundary that must be argued gets argued, and always toward the cheaper tool. So there is no boundary now.

**A degraded critique does not satisfy this gate.** `.agents/skills/impeccable/reference/critique.md` (the committed copy — cite that one, not `~/.claude/…`, so the rule is verifiable from a clone; lines 8 and 35 were checked to be byte-identical in both builds) requires Assessment A and B to run as two isolated sub-agents whenever a sub-agent tool is exposed, and requires a `⚠️ DEGRADED: single-context` banner otherwise. The legitimate triggers are exactly two, both from that file: no sub-agent/Task tool is exposed in the session, or — on harnesses that ask — the user declined. Neither describes a controller choosing not to spawn them; the file says outright, *"It does not mean inconvenient."* When dispatching the critique to a sub-agent, never paste a no-subagents instruction into its prompt: that contract belongs to *implementer* agents, and pasted into a critique agent it manufactures the degradation itself. This is not hypothetical — it is exactly how PR #759's 32/40 came to be degraded. Report a degraded score as degraded, or re-run it properly.
...
The `/review` subagent must also run the full `critique` when the diff includes HTML or CSS, and must flag as a **FAIL** any PR whose description names a tool the author did not run, reports a degraded score as a clean one, or claims a lighter tier that no longer exists. An impeccable FAIL is a merge blocker — same weight as a broken link.
...
1b. **Deterministic detector (`detect.mjs`), on its own:** **no longer a permitted substitute for the gate, in any circumstance** (owner decision 2026-08-20; it previously stood in for copy-only diffs). It catches the mechanical anti-patterns (gradient text, side-stripes, glassmorphism, contrast) and nothing that needs judgment. It still runs — inside the critique as Assessment B, and in the per-edit hook below — but never *instead of* the critique.
   - ⚠️ **It does NOT enforce this project's em-dash ban, and you must not assume it does.** Its rule is `em-dash-overuse` and fires only at **5 or more** em dashes in a file (`.agents/skills/impeccable/scripts/detector/engines/regex/detect-text.mjs`), because upstream treats occasional em dashes as normal prose. This project bans them outright in editorial copy. So 1–4 em dashes pass the detector clean while violating the rule. **On every HTML/CSS PR, grep the changed files yourself** (`grep -n '—' <changed-files>`, see the Em dashes pattern above) and say in the PR that you did. Caught in review of the PR that introduced this tier. The tier is gone as of 2026-08-20, but this duty is not tier-dependent and survives it: the detector still cannot settle the em-dash question, so the grep is still yours to run, on every HTML/CSS PR.
...
```
npm test                 # link check (every page) + integrity + CSS vars + repo hygiene (large-file guard) + formspree-guard (contact-form route-stub check) + partial drift check (footer/nav) + site.js drift check + blog-count drift check + article-byline drift check
npm run screenshot       # Playwright batch screenshots
...
- `/review` — review changed files before merging. **Must flag as FAIL if `npm run test:functional` is not confirmed passing. Must run the full `/impeccable critique` on any changed HTML/CSS page (no detector-only tier exists), flag impeccable FAILs as merge blockers, and FAIL any PR that names a tool the author did not run or reports a degraded score as a clean one.**
