...
**A degraded critique does not satisfy this gate.** `~/.claude/skills/impeccable/reference/critique.md` requires Assessment A and B to run as two isolated sub-agents whenever a sub-agent tool is exposed, and requires a `⚠️ DEGRADED: single-context` banner otherwise. The only legitimate trigger is that no sub-agent tool exists — that file says outright, *"It does not mean inconvenient."* When dispatching the critique to a sub-agent, never paste a no-subagents instruction into its prompt: that contract belongs to *implementer* agents, and pasted into a critique agent it manufactures the degradation itself. This is not hypothetical — it is exactly how PR #759's 32/40 came to be degraded. Report a degraded score as degraded, or re-run it properly.
