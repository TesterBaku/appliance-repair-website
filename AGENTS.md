...
**CSS background-image heroes** (hub pages) require an explicit `<link rel="preload" as="image">` in `<head>` — the browser preload scanner cannot discover them from CSS. Use `scripts/add-hero-preload.mjs` to backfill. **Enforced since 2026-08-20 by the `hero-preload` check in `test/content-integrity.js`** (check 23): every page whose CSS declares a `background-image: url(...)` on `.hub-hero-bg` must carry a preload `<link>` whose `href` resolves to that same file. It resolves both the CSS `url()` and the `href` against the page's own directory rather than comparing raw strings, so two different relative paths to one file match and two different files sharing a basename do not. Until then the rule was documented but unchecked, and `pages/services.html` had quietly regressed: its hero loaded `hero-homepage.webp` while its only preload was for `shared.css`. Live coverage: 69 pages.
