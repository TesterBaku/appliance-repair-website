...
1b. **Deterministic detector (`detect.mjs`), on its own:** the permitted substitute for the gate **only** on copy-only diffs. It catches the mechanical anti-patterns (gradient text, side-stripes, glassmorphism, em dashes, contrast) and nothing that needs judgment. Sufficient for a price or wording change; not sufficient for a new section.
